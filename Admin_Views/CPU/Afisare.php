...
if (!empty($descriere)) {
    ...
    $stmt = $conn->prepare("SELECT * FROM CPU WHERE descriere LIKE ?");
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>" . htmlspecialchars($row["nucleu"], ENT_QUOTES) . "</td>";
        echo "<td>" . htmlspecialchars($row["frecventa"], ENT_QUOTES) . "</td>";
        echo "<td>" . htmlspecialchars($row["tehnologie_fabricatie"], ENT_QUOTES) . "</td>";
        ...
        echo "<td>
                ...
                <a href='Admin_CPU_ModificareForm.php?id=" . $row["id"] . "'>modifică</a><br>
              </td>";
        echo "</tr>";
    }
} else {
