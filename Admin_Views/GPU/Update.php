...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES['imagine']['name'])) {
        ...
        $query = "SELECT imagine FROM GPU WHERE id = ?";
    ...
    $sql = "UPDATE GPU SET descriere=?, seria=?, boost_clock=?, tip_memorie=?, dimensiune_memorie=?, pret=?, imagine=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssssi", $descriere, $seria, $boost_clock, $tip_memorie, $dimensiune_memorie, $pret, $imagine, $id);
    
    // Executarea interogării
    if ($stmt->execute()) {
        echo "Înregistrarea a fost actualizată cu succes.";
        header("Location: Admin_GPU_tab.php"); // Redirect către pagina de administrare
    } else {
