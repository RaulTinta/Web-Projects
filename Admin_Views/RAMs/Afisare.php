...
if (!empty($descriere)) {
    ...
    $stmt = $conn->prepare("SELECT * FROM RAMs WHERE descriere LIKE ?");
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>" . htmlspecialchars($row["tip"], ENT_QUOTES) . "</td>";
        echo "<td>" . htmlspecialchars($row["capacitate"], ENT_QUOTES) . "</td>";
        echo "<td>" . htmlspecialchars($row["frecventa"], ENT_QUOTES) . "</td>";
        echo "<td>" . htmlspecialchars($row["latenta_cas"], ENT_QUOTES) . "</td>";
        ...
        echo "<td>
                ...
                <a href='Admin_RAMs_ModificareForm.php?id=" . $row["id"] . "'>modifică</a><br>
              </td>";
        echo "</tr>";
    }
} else {
