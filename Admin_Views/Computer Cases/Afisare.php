...
if (!empty($descriere)) {
    ...
    $stmt = $conn->prepare("SELECT * FROM computercases WHERE descriere LIKE ?");
    $stmt->bind_param("s", $descriere);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    // Altfel, afișează toate datele
    $sql = "SELECT * FROM computercases";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>" . htmlspecialchars($row["tip_carcasa"], ENT_QUOTES) . "</td>";
        echo "<td>" . htmlspecialchars($row["dimensiune"], ENT_QUOTES) . "</td>";
        echo "<td>" . htmlspecialchars($row["sloturi_expansiune"], ENT_QUOTES) . "</td>";
        ...
        echo "<td>
                ...
                <a href='Admin_ComputerCases_ModificareForm.php?id=" . htmlspecialchars($row["id"], ENT_QUOTES) . "'>modifică</a><br>
              </td>";
        echo "</tr>";
    }
} else {
