...
if (!empty($descriere)) {
    ...
    $stmt = $conn->prepare("SELECT * FROM motherboards WHERE descriere LIKE ?");
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>" . htmlspecialchars($row["soclu_procesor"], ENT_QUOTES) . "</td>";
        echo "<td>" . htmlspecialchars($row["chipset_producator"], ENT_QUOTES) . "</td>";
        echo "<td>" . htmlspecialchars($row["chipset_model"], ENT_QUOTES) . "</td>";
        ...
        echo "<td>
                ...
                <a href='Admin_MotherBoards_ModificareForm.php?id=" . $row["id"] . "'>modifică</a><br>
              </td>";
        echo "</tr>";
    }
} else {
